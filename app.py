...
def home():
    ...
    if role == "admin":
        ...
        counts = {
            ...
            "student_unread": 0, "student_received": 0,
            "sent_students": 0, "sent_admin": 0,
            "sent_circulars": 0, "sent_homework": 0,
            "news_unread": 0, "news_total": 0,
        }
        students = db.execute("""
        ...
        counts["inbox_unread"] = db.execute("""
            ...
            WHERE m.recipient_id = ? AND m.is_read = 0 AND u.role IN ('admin','teacher')
        """, session["user_id"])[0]["c"]
        counts["inbox_received"] = db.execute("""
            SELECT COUNT(*) AS c FROM messages m
            JOIN users u ON m.sender_id = u.id
            WHERE m.recipient_id = ? AND u.role IN ('admin','teacher')
        """, session["user_id"])[0]["c"]
        counts["student_unread"] = db.execute("""
            SELECT COUNT(*) AS c FROM messages m
            JOIN users u ON m.sender_id = u.id
            WHERE m.recipient_id = ? AND m.is_read = 0 AND u.role = 'student'
        """, session["user_id"])[0]["c"]
        counts["student_received"] = db.execute("""
            SELECT COUNT(*) AS c FROM messages m
            JOIN users u ON m.sender_id = u.id
            WHERE m.recipient_id = ? AND u.role = 'student'
        """, session["user_id"])[0]["c"]
        ...
        counts["sent_admin"] = db.execute("""
            SELECT COUNT(*) AS c FROM messages m
            JOIN users u ON m.recipient_id = u.id
            WHERE m.sender_id = ? AND u.role = 'admin'
        """, session["user_id"])[0]["c"]
        counts["sent_circulars"] = db.execute(
            "SELECT COUNT(*) AS c FROM circulars WHERE sender_id = ?", session["user_id"]
        )[0]["c"]
        counts["sent_homework"] = db.execute(
            "SELECT COUNT(*) AS c FROM homework WHERE sender_id = ?", session["user_id"]
        )[0]["c"]
        counts["news_total"] = db.execute("SELECT COUNT(*) AS c FROM news")[0]["c"]
        counts["news_unread"] = db.execute("""
            SELECT COUNT(*) AS c FROM news n
            WHERE n.id NOT IN (SELECT news_id FROM news_seen WHERE user_id = ?)
        """, session["user_id"])[0]["c"]
        ...
        error = ""
        counts = {
            "total": 0, "inbox_unread": 0, "inbox_total": 0,
            "sent_total": 0, "circulars_unread": 0, "circulars_total": 0,
            "news_unread": 0, "news_total": 0,
            "homework_unread": 0, "homework_total": 0,
        }
        ...
        if request.method == "POST" and not info_submitted:
            ...
            if name and grade in VALID_GRADES and section in VALID_SECTIONS and dob:
                db.execute(
                    "INSERT INTO grades (user_id, name, grade, section, dob) VALUES (?, ?, ?, ?, ?)",
                    session["user_id"], name, grade, section, dob
                )
                info_submitted = True
        ...
        if student_info:
            ...
            counts["circulars_unread"] = db.execute("""
                SELECT COUNT(*) AS c FROM circulars c
                WHERE c.grade = ? AND c.id NOT IN (
            ...
            counts["circulars_total"] = db.execute(
                "SELECT COUNT(*) AS c FROM circulars WHERE grade = ?", student_info["grade"]
            )[0]["c"]
            counts["homework_unread"] = db.execute("""
                SELECT COUNT(*) AS c FROM homework h
                WHERE h.grade = ? AND h.id NOT IN (
                    SELECT homework_id FROM homework_seen WHERE user_id = ?
                )
            """, student_info["grade"], session["user_id"])[0]["c"]
            counts["homework_total"] = db.execute(
                "SELECT COUNT(*) AS c FROM homework WHERE grade = ?", student_info["grade"]
            )[0]["c"]
            counts["news_unread"] = db.execute("""
                SELECT COUNT(*) AS c FROM news n
                WHERE n.id NOT IN (SELECT news_id FROM news_seen WHERE user_id = ?)
            ...
            counts["news_total"] = db.execute("SELECT COUNT(*) AS c FROM news")[0]["c"]
            counts["total"] = (
                counts["inbox_unread"] + counts["circulars_unread"]
                + counts["homework_unread"] + counts["news_unread"]
            )
...
def student_circulars():
    if not db.execute("SELECT 1 FROM grades WHERE user_id = ? LIMIT 1", session["user_id"]):
        return redirect(url_for("home", need_info=1))
    grade = db.execute(
        "SELECT grade FROM grades WHERE user_id = ? LIMIT 1", session["user_id"]
    )[0]["grade"]
...
def teacher_circulars():
    ...
    circulars = db.execute("""
        ...
        FROM circulars WHERE sender_id = ?
...
def admin_dashboard():
    ...
    admin_counts = {
        "inbox_unread": db.execute(
            "SELECT COUNT(*) AS c FROM messages WHERE recipient_id = ? AND is_read = 0",
            session["user_id"])[0]["c"],
        "inbox_received": db.execute(
            "SELECT COUNT(*) AS c FROM messages WHERE recipient_id = ?",
            session["user_id"])[0]["c"],
        "sent_students": db.execute("""
            SELECT COUNT(*) AS c FROM messages m
            JOIN users u ON m.recipient_id = u.id
            WHERE m.sender_id = ? AND u.role = 'student'
        """, session["user_id"])[0]["c"],
        "sent_teachers": db.execute("""
            SELECT COUNT(*) AS c FROM messages m
            JOIN users u ON m.recipient_id = u.id
            WHERE m.sender_id = ? AND u.role = 'teacher'
        """, session["user_id"])[0]["c"],
        "sent_news": db.execute(
            "SELECT COUNT(*) AS c FROM news WHERE sender_id = ?",
            session["user_id"])[0]["c"],
        "sent_circulars": db.execute(
...
def admin_grades():
    ...
    if request.method == "POST":
        ...
        students = db.execute("""
            ...
            JOIN grades g ON u.id = g.user_id
            WHERE g.grade = ? ORDER BY u.username
        """, selected_grade)
    else:
        students = db.execute("""
            SELECT u.username, g.grade, u.id FROM users u
