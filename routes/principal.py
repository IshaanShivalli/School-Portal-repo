...
def principal_messages():
    ...
    if school_id:
        teachers = db.execute(
            "SELECT id, username FROM users WHERE role = 'teacher' AND school_id = %s ORDER BY username",
            school_id
        )
        students = db.execute(
            "SELECT id, username FROM users WHERE role = 'student' AND school_id = %s ORDER BY username",
            school_id
        )
